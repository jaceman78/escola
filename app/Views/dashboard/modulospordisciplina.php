...
let lista = document.querySelector('ul[name="itemprofissional"]');
...
lista.appendChild(novoItem);                                                
  ...
  // dataTables
  $(function() {  
    $('.card-title').text('Módulos da disciplina de '+'<?= $nomedadisciplina ;?>') ; 
    var table = $('#data_table').removeAttr('width').DataTable({
      ...
      "ajax": {
        "url": '<?php echo base_url($controller . "/getAllpordisciplina/".$iddisciplina) ?>',
        "type": "POST",
        "dataType": "json",
        async: "true"
      }
  ...
  function save(id_modulo) {
    // reset the form 
    $("#data-form")[0].reset();
    $(".form-control").removeClass('is-invalid').removeClass('is-valid');
    if (typeof id_modulo === 'undefined' || id_modulo < 1) { //add
      urlController = '<?= base_url($controller . "/add") ?>';
      submitText = '<?= lang("App.save") ?>';
      $('#model-header').removeClass('bg-info').addClass('bg-success');
      $("#info-header-modalLabel").text('<?= lang("App.add") ?> Módulo ');
